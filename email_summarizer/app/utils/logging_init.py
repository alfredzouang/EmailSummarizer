...
_bootstrap_logger = logging.getLogger("email_summarizer.logging_manager")
...
class EmailSummarizerLogging:
    """EmailSummarizer统一日志管理器"""
    ...
    def _load_default_config(self) -> Dict[str, Any]:
        ...
        # 从环境变量获取配置
        log_level = os.getenv('EMAIL_SUMMARIZER_LOG_LEVEL', 'INFO').upper()
        log_dir = os.getenv('EMAIL_SUMMARIZER_LOG_DIR', './logs')
        ...
        return {
            ...
            'loggers': {
                'emailsummarizer': {'level': log_level},
    ...
    def _add_structured_handler(self, logger: logging.Logger):
        ...
        log_file = log_dir / 'email_summarizer_structured.log'
...
# 全局日志管理器实例
_logger_manager: Optional[EmailSummarizerLogging] = None


def get_logger_manager() -> EmailSummarizerLogging:
    """获取全局日志管理器实例"""
    global _logger_manager
    if _logger_manager is None:
        _logger_manager = EmailSummarizerLogging()
...
def setup_logging(config: Optional[Dict[str, Any]] = None):
    """设置项目日志系统（便捷函数）"""
    global _logger_manager
    _logger_manager = EmailSummarizerLogging(config)
    return _logger_manager
